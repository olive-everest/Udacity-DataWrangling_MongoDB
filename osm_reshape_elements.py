    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        for tag in element.iter('tag') :     
            if tag.attrib['k'].startswith('addr:') :
                #print tag.attrib['k']
                key = re.sub('addr:', '', tag.attrib['k'])
                #print key
                # ignore double colon keys like addr:street:number
                if (lower_colon.search(key)) :
                    ...
                    #print "%s : %s" %(key, tag.attrib['v'])
            elif  (problemchars.search(tag.attrib['v'])) :
                # Ignore any kv pairs with problematic chars
                dummy = 1
            else :
...
def test():
    ...
    #print data[0]
    #print "Should be:  ", correct_first_elem
